...
## Run tracking & experiment reproducibility

Backtest runs can now be tracked automatically by appending metadata to a CSV ledger:
...
* Conda recipe lives under `conda/`. Build it with `conda build conda` to produce an
  installable package containing the optimizer and CLI dependencies.
* A CPU-only Docker image can be produced with `docker build -t neuro-ant-optimizer .`.
  The image is based on `python:3.11-slim`, installs the project in editable form, and
  exposes the backtest CLI as its default command.

Example notebooks showcasing the quickstart outputs and factor diagnostics are located in
`examples/notebooks/` and align with the artifacts generated by the scripted quickstart.

---

## Developer quickstart workflow

Use the make target to download (or fall back to bundled) sample data, run a fully tracked
backtest, and generate replayable artifacts:

```bash
make quickstart
```

The command writes artifacts to `quickstart_artifacts/`, appends to `runs.csv`, and stores
...
exercise key numerical invariants (simplex projections, PSD repair, tracking error, and
drawdown bounds). Run them via `pytest` to ensure regressions are caught, and leverage
`mutmut` for mutation testing to monitor kill-rate improvements when iterating on the
core maths.
